...
1. 零钱罐子：罐子管理、资金池、分配记录与建议，入口在首页标题右侧
2. 多用户改造任务清单：新增，按「配置 → 中间件 → 数据层 → API → 前端」拆解执行步骤
   - 方案要点：固定试用账号（令牌→user_id，无注册）；旁观者仅 GET 可看主账号、写操作 401；入口为 URL `type`/`j-user-id` 或 localStorage，请求头 `j-user-id`；config.env 维护 MAIN_USER_ID 与令牌映射，支持撤销/轮换；所有表加 user_id，唯一约束含 user_id

---

## 2026.01.18

1. 运动类型管理，将硬编码的运动类型迁移到数据库 `routine_type` 表
2. 月报页面 URL 参数 `monthId` 支持
